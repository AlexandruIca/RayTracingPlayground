...
auto main() -> int
{
    ...
    constexpr int vfov = 25.0;

    camera_parameters params{};
    params.lookfrom = point3{ -2, 2, 1 };
    params.lookat = point3{ 0, 0, -1 };
    params.up = point3{ 0, 1, 0 };
    params.vert_fov = vfov;
    params.aperture = 0.2; // NOLINT
    params.focus_distance = glm::length(params.lookfrom - params.lookat);

    camera cam{ params };
    hittable_list world{};

    auto const yellowish = color{ 0.8, 0.8, 0.0 };
    ...
    auto material_left = std::make_shared<dielectric>(refraction_index);
    auto material_right = std::make_shared<metal>(goldish, fuzz_factor);
    ...
    world.add(std::make_unique<sphere>(point3{ -1.0, 0.0, -1.0 }, 0.5, material_left));     // NOLINT
    world.add(std::make_unique<sphere>(point3{ 1.0, 0.0, -1.0 }, 0.5, material_right));     // NOLINT
